...
// SlimVolgaConfig is a memory-constrained profile for the iOS Network Extension,
// which is killed if it exceeds ~40MB. DefaultVolgaConfig's 2000 workers /
// 1M-entry queues are a server profile that blows that cap; a single mobile
// tunnel needs only a handful of relay workers and a small queue. The batching
// wire format is identical, so this interoperates with a full-config exit node.
...
var (
	// 256KB initial cap: base64Encode grows it on demand for larger batches,
	// so a big server batch still works, but a memory-constrained client (iOS
	// NE) keeps only small pooled buffers instead of 16MB each.
...
// NewYandexVolgaTransportWithConfig builds a VOLGA transport with an explicit
// resource profile (see SlimVolgaConfig for memory-constrained clients).
