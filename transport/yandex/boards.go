...
type boardsSession struct {
	...
	// creatorHash — то, что кладём в _attributes_.creatorHash в
	// modify-objects. Изначально = participant, но если сервер в
	// participant-connected/431 прислал свой хэш — берём его.
...
// getWhiteboardSolvingCaptcha — GET /whiteboard с автопрохождением капчи.
// После решения повторяет GET: сервер выдаёт свежие cookies, нужные
// последующим /api запросам.
func (t *BoardsTransport) getWhiteboardSolvingCaptcha(client *http.Client, docURL, hash string) error {
	err := t.getAllowCaptcha(client, docURL, hash)
	if err == nil {
		return nil
	}
	...
	if _, cerr := solveCaptcha(docURL, client.Jar, boardsUA); cerr != nil {
		return fmt.Errorf("captcha solve: %w", cerr)
	}
	...
	if err := t.getAllowCaptcha(client, docURL, hash); err != nil && err != errCaptchaRequired {
		return fmt.Errorf("GET whiteboard (post-captcha): %w", err)
	}
	return nil
}
...
func (t *BoardsTransport) authorize(hash, name string) (boardsInfo, error) {
	...
	if err := t.getWhiteboardSolvingCaptcha(client, docURL, hash); err != nil {
		return boardsInfo{}, fmt.Errorf("GET whiteboard: %w", err)
	}
	...
	if err := t.getWhiteboardSolvingCaptcha(client, docURL, hash); err != nil {
		return boardsInfo{}, fmt.Errorf("GET whiteboard (jwt): %w", err)
	}
...
// newAPIRequest собирает POST /api.
//
// ВАЖНО: с 2026-09-24 сервер отвечает 415 ("Request body must use a JSON
// Content-Type") на старый application/x-www-form-urlencoded. Тело теперь
// JSON {"action":..., "content":"<base64 от JSON-параметров>"}; сама
// base64-обёртка content осталась прежней.
func newAPIRequest(hash, action string, content interface{}) *http.Request {
...
func (t *BoardsTransport) postAPI(client *http.Client, hash, action string, content interface{}) error {
	resp, err := client.Do(newAPIRequest(hash, action, content))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func (t *BoardsTransport) getWhiteboardInfo(client *http.Client, hash string) (map[string]string, error) {
	resp, err := client.Do(newAPIRequest(hash, "get-whiteboard-info",
...
// handshake — engine.io + socket.io + subscribe-slide-dashboard.
//
// ВАЖНО: при ошибке чтения ВОЗВРАЩАЕМ err, а не break. Если сделать break,
// gorilla/websocket закэширует readErr в Conn, и первый же ReadMessage
// в основном цикле мгновенно вернёт тот же i/o timeout.
...
// ---- keepalive ----
//
// Heartbeat как в Python-скрипте: cmd_hb() -> send_dashboard("heartbeat", {}),
// и send_dashboard добавляет participant в envelope. То есть:
//   42N["dashboard",{"action":"heartbeat","data":{},"participant":"<hash>"}]
//
// Плюс отдельно engine.io ping "2" (сервер отвечает "3"), чтобы NAT не
// рвал idle-соединение.
...
// NB: engine.io v4 — пингует СЕРВЕР пакетом "2", клиент отвечает "3"
// (в v3 было наоборот). Свой ping-loop с writeRaw("2") сервер считает
// невалидным пакетом от клиента и рвёт сокет ровно через boardsPingInterval.
// Ответ "3" на серверный пинг живёт в handleMessage — этого достаточно,
// keep-alive поверх него даёт heartbeat в namespace dashboard.
...
func (t *BoardsTransport) handleMessage(sess *boardsSession, raw []byte) {
	...
	// 43N[...] — ack'и, могут нести dashboard_link / participantHash
...
// handleParticipantConnected — запоминаем серверный хэш как creatorHash.
// participant в subscribe НЕ меняем — иначе сервер повторно шлёт
// participant-connected, а мы зацикливаемся.
...
// handleServerModifyObjects — основной канал приёма.
//
// Фильтруем СВОИ объекты по creatorHash (userHash / participant / creatorHash)
// и по name. Иначе получаем эхо собственных modify-objects.
func (t *BoardsTransport) handleServerModifyObjects(sess *boardsSession, raw json.RawMessage, action string) {
	...
	if err := json.Unmarshal(raw, &d); err != nil {
		utils.Debugf("[BOARDS] %s unmarshal error: %v", action, err)
		return
	}
	if len(d.Objects) == 0 {
		return
	...
	for _, o := range d.Objects {
		...
		// Своё эхо: creatorHash совпадает с любым из наших идентификаторов,
		// ИЛИ отправитель — мы сами. ВАЖНО: creatorHash не должен совпадать
		// с чужим, поэтому myCreator в этом сравнении — это ТОЛЬКО наш хэш,
		// а не общий "владелец доски".
...
// handle431 — ack'и на наши 42N[...]. Ищем dashboard_link, session,
// participantHash, creatorHash.
